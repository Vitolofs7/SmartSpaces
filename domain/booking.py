...
class Booking:
    ...
    def __init__(self, booking_id, space, user, start_time, end_time):
        ...
        self._status = Booking.STATUS_ACTIVE
    ...
    def is_active(self):
        return self._status == Booking.STATUS_ACTIVE

    def cancel(self):
        if self._status != Booking.STATUS_ACTIVE:
            raise ValueError("Only active bookings can be cancelled.")
        self._status = Booking.STATUS_CANCELLED

    def finish(self):
        if self._status != Booking.STATUS_ACTIVE:
            raise ValueError("Only active bookings can be finished.")
        self._status = Booking.STATUS_FINISHED
